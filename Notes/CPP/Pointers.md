...
int main() {
    ...
    ptr = nullptr; // We can assign a null value to a pointer, so that we donot accidently acess a memory which we are not suppose to. This is a null pointer which is a pointer which doesn't point to anything. In older versions of cpp, we used 'NULL' or '0', to initialize the pointer, and make it a null pointer.
...
int main() {
    const int x = 10;
    const int* ptr = &x; // We read this as pointer to a constant integer. This will give compilation error if we do int* ptr = &x; Here the data is constant but pointer is not so later we can use the same pointer to point to some other memory loaction. const int* ptr allows the pointer to point to different locations, but the values being pointed to cannot be modified through this pointer.
...
int main() {
    int x = 10;
    int* const ptr = &x; // We should initialize the constant pointer after declearing because latere we cannot change it and it will be assigned to some garbage memory.
...
void printNumbers(int numbers[]) {
    ...
    size(numbers); // numbers holds a memory address, so we cannot pass integer array numbers to the size function.
...
int main() {
    ...
    int* ptr = numbers;
    cout << numbers << " " << ptr;  // Output: hexa-decimal address first elemnt of the array.
    
    cout << *numbers << " " << *ptr;  // Output: 10 10 - first elemnt of the array.

    cout << number[2] << " " << ptr[2]; // Output: 30 - third element of the array.
}
...
int main() {
    ...
    cout << *(ptr + 1) << " " << ptr[1] << " " << numbers[1]; // Output: 10 10 10 - *(prt + 1) == ptr[1] == numbers[1]
```
...
int main() {
    ...
    if (prtX == ptrY) // Here we are comparing two memory address. If both the pointers were pointing to the same memory loaction then we can also use this to confirm that values at the memory loaction where both the pointers are pointing. 

    if (*prtX == *ptrY) // Here we are comparing values at the memory loaction where both the pointers are pointing. 
}
```
## Dynamic memmory allocation
```cpp
// Stack, here we don't need clean up
int numbers[1000];

// Heap
int* numbers = new int[10];
// Once we used this variable we have to free up the memory or the system will comsume more and more memory and eventually system will crash. This is know as memory leak.
