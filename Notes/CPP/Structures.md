...
int main() {
    // If we don't initialize this instace it will hold some garbage value. There are two ways of initializing an instace.
    // 1st method
...
ostream& operator<<(ostream& stream, const Movie& movie) {
    stream << movie.title; // Here we are just writing just one member of the object.
